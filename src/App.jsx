...
import {Logo, LogoBookmark, LogoMasterCraft, MenuClose, MenuOpen} from "./icons";
...
function App() {
    ...
    // let  menuVariants = window.innerWidth <= `${desktopView}px` ? {
    //     hidden: {
    //         scale: 0,
    //         opacity: 0
    //     },
    ...
    return (
        <>
            <picture className={'picture'}>
                <source className={"picture__desktop"} srcSet={desktopViewImg} media={`(min-width:${desktopView}px)`}/>
                <source className={"picture__mobile"} srcSet={mobileViewImg} media={`(min-width:${mobileView}px)`}/>
                <img className={"picture__mobile"} src={mobileViewImg} alt=""/>
            </picture>
            <header className={"header"}>
                <Logo/>
                <div className="header__menu-icon">
                    <motion.div className={"header__close"} initial={menuIconInitial}
                                animate={isMenuShowing ? menuIconAnimate : {}} onClick={handleMenu}><MenuClose/>
                    </motion.div>
                    <motion.div className={"header__open"} initial={menuIconInitial}
                                animate={!isMenuShowing ? menuIconAnimate : {}} onClick={handleMenu}><MenuOpen/>
                    </motion.div>
                </div>
                <motion.ul
                    ...
                    <motion.li variants={menuVariants}  className="header__menu-option">About
                    </motion.li>
                    <motion.li variants={menuVariants}
                               className="header__menu-option">Discover
                    </motion.li>
                    <motion.li variants={menuVariants}  className="header__menu-option">Get
                        Started
                    </motion.li>
                </motion.ul>
            </header>
            ...
            <main className={"main"}>
                <div className="intro ">
                    ...
                    <div className="intro__btns-container">
                        <motion.button whileTap={{scale: .9}} className="intro__back-btn">Back this project
                        </motion.button>
                        <motion.button style={isBookmarked && {color:"#147B74FF"}} onClick={handleBookmark}
                                       whileTap={{scale: .9}} className="intro__bookmark-btn"><LogoBookmark
                            circleFill={isBookmarked && bookmarkCircleActive}
                            labelFill={isBookmarked && bookmarkLabelActive}/><span
                            className="intro__bookmark-txt">Bookmark</span></motion.button>
                    </div>
                </div>
                ...
                <div className="info ">
                    ...
                    <ul className="info__values">
                        <li className="info__parameter">
                            <h2 className="info__value">$<AnimatedAmount duration={1.5} from={0} to={currentIncome}/></h2>
                            <p className="info__desc">of ${totalIncome.toLocaleString()} backed</p>
                        </li>
                        <hr className={"info__bar"}/>
                        <li className="info__parameter">
                            <h2 className="info__value"><AnimatedAmount from={0} to={currentBackers} duration={1}/></h2>
                            <p className="info__desc">total backers</p>
                        </li>
                        <hr className={"info__bar"}/>
                        <li className="info__parameter">
                            <h2 className="info__value"><AnimatedAmount from={100} to={daysLeft} duration={.5}/></h2>
                            <p className="info__desc">days left</p>
                        </li>

                    </ul>
                    {/*<progress className={"info__prog"} value={currentIncome} max={totalIncome}/>*/}
                    <AnimatedProgress from={0} to={currentIncome} max={totalIncome} duration={1.5}/>
                </div>
            ...
            <motion.div onClick={handleMenu} initial={{opacity: 0, zIndex: -1}}
                        animate={isCoverShowing ? {opacity: 1, zIndex: 1} : {}} transition={{duration: .05}}
                        className="cover"/>
